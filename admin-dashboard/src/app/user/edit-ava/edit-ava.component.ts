    ...
    if (this.selectedFile && this.imageError == null) {
      this.commonService.uploadImage(this.selectedFile!).subscribe({
        next: (stringNameAvatar: string) => {
          ...
          console.log(data);
          this.commonService.editAvaFile('users/change/avatar',data).subscribe({
            next: (response) => {
              console.log('avatar file updated successfully:', response);
              this.toastr.success('', 'updated avatar file!');
              this.dialogRef.close('saved');
            },
            error: (error) => {
              console.error('Error updating name avatar:', error);
              this.toastr.error('', 'Error updating name avatar file!');
            },
          });
        },
        error: (error) => {
