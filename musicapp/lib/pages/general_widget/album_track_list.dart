...
class AlbumTrackList extends StatelessWidget {
  ...
  const AlbumTrackList({Key? key, required this.tracks}) : super(key: key);
  ...
  void goToSong(BuildContext context, int songIndex) {
    ...
    playlistProvider.currentSongIndex =
        songIndex; // Cập nhật index bài hát hiện tại
    print(
        'Current Song Index: ${playlistProvider.currentSongIndex}'); // Kiểm tra giá trị của currentSongIndex
  ...
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final isLiked = context.watch<SongProvider>().isLiked(track.id);
